resource "aws_instance" "silvex-centos-8-100-tf" {
  ami           = var.SILVEX-AMI
  ...
  user_data = "${file("./setup-ec2.sh")}"
    ...
    tags = {
    Name = "silvex-centos-8-100-tf"
  }

}
resource "aws_instance" "silvex-centos-8-101-tf" {
  ami           = var.SILVEX-AMI
  instance_type = var.SILVEX-EC2-TYPE
  key_name = var.SILVEX-SSH-2020
  ...
  user_data = "${file("./setup-ec2.sh")}"
    ...
    tags = {
    Name = "silvex-centos-8-101-tf"
  }

}
resource "aws_instance" "silvex-centos-8-102-tf" {
  ami           = var.SILVEX-AMI
  instance_type = var.SILVEX-EC2-TYPE
  key_name = var.SILVEX-SSH-2020
